...
object WechatGlobal {
     ...
     */
    @JvmStatic fun init(lpparam: XC_LoadPackage.LoadPackageParam, callback:() -> Unit) {
        tryAsynchronously {
            ...
            try {
                XposedBridge.log("WechatGlobal init")
                wxVersion = getApplicationVersion(lpparam.packageName)
                wxPackageName = lpparam.packageName
                wxLoader = lpparam.classLoader
                XposedBridge.log("WechatGlobal init  lpparam.appInfo.sourceDir = ${lpparam.appInfo.sourceDir}")
                ApkFile(lpparam.appInfo.sourceDir).use {
                    XposedBridge.log("WechatGlobal init  apkfile = $it")
                    XposedBridge.log("WechatGlobal init  classtypes = ${it.classTypes}")
                    wxClasses = it.classTypes
                    if (initChannel.isDone()){
                        XposedBridge.log("WechatGlobal init  callback")
                        callback()
                    }
                }
            } finally {
