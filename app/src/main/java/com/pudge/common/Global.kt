...
object Global {
    ...
    const val PUDGE_PACKAGE_NAME = "com.pudge"

    const val XPOSED_FILE_PROVIDER   = "$XPOSED_PACKAGE_NAME.fileprovider"
    const val MAGICIAN_FILE_PROVIDER = "$PUDGE_PACKAGE_NAME.files"
    ...
    val PUDGE_BASE_DIR = "$DATA_DIR/$PUDGE_PACKAGE_NAME/"

    const val FOLDER_SHARED_PREFS = "shared_prefs"

    const val PREFERENCE_PROVIDER_AUTHORITY   = "com.pudge.preferences"
    const val PREFERENCE_NAME_SETTINGS        = "settings"
    const val ACTION_UPDATE_PREF         = "$PUDGE_PACKAGE_NAME.ACTION_UPDATE_PREF"
